...
@WebServlet("/ForgetPasswordMailVerifier")
public class ForgetPasswordMailVerifier extends HttpServlet 
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		org.hibernate.Session ss = SSConnector.getSession();
		...
		if(l1 != null)
		{
		  for(AdminDetails ad : l1)
		  {
			mailCheck = ad.getAdminEmail();  
		  }
		}
		
		if(mailCheck.equals(to))
		{
		  ...
		  response.sendRedirect("FillOTP.jsp");
		}
		else
		{
		  session.setAttribute("failed", "Your mailid is not matched with your account id!");
